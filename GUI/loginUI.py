...
def runLoginUi():
    ...
    def verifyButton(usernameInput, passwordInput, root):
        ...
        if verification == True:
            success = Button(root, text="SUCCESS!", bg="black", fg="white",width=40, height=10)
            success.grid(row=1, column=2)
        else:
            failed = Button(root, text="LOGIN FAILED!", bg="black", fg="white",width=40, height=10)
            failed.grid(row=1, column=2)
